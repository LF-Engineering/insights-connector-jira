...
export AWS_ACCESS_KEY_ID="`cat ./secrets/AWS_ACCESS_KEY_ID.lukasz.secret`"
export AWS_SECRET_ACCESS_KEY="`cat ./secrets/AWS_SECRET_ACCESS_KEY.lukasz.secret`"
...
#./jira --jira-url='https://jira.riscv.org' --jira-debug=0 --jira-es-url="${ESURL}" --jira-user="`cat ./secrets/user.encrypted.secret`" --jira-token="`cat ./secrets/token.encrypted.secret`" --jira-stream="${STREAM}" $* 2>&1 | tee run.log
...
./jira --jira-url='https://jira.pnda.io' --jira-debug=2 --jira-es-url="${ESURL}" --jira-user='' --jira-token='' --jira-stream="${STREAM}" --jira-date-from="2018-01-10" $* 2>&1 | tee run2.log
